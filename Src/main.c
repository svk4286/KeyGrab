...
extern uint8_t uid[], uidLength, nuid;  //  UID
extern char Tx[512];
...
int main(void)
{
  ...
  while( !readPassiveTargetID(&hspi1,PN532_MIFARE_ISO14443A, uid, &uidLength) ){	// Ожидание ключа
	  tmp ='.';
  ...
  for(i = 0, uid[uidLength] = 0; i < uidLength; i++){   //Вычисление BCC для UID
	  uid[uidLength] ^= uid[ i ];
  }
    nuid = uidLength + 1;
